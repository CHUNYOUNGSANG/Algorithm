...
public class No25314_CodingIsAPhysicalEducationSubject {    // 백준 제출할 때는 Main 으로 변경해야 함.
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);    // 사용자를 입력 받기 위해 Scanner 객체를 생성한다.
        int N = sc.nextInt();   // 정수 N을 입력 받음.
        int a = N / 4;  // N을 4로 나눈 몫을 계산하여 a에 저장.

        for (int i = 0; i < a; i++) {   // a만큼 반복함.
            System.out.print("long ");  // "long "을 출력한다.
        }
        System.out.println("int");  // "int"를 출력함.
        sc.close(); // Scanner 객체를 닫아 메모리 누수를 방지한다.
    }
}

/*
 문제 : 오늘은 혜아의 면접 날이다. 면접 준비를 열심히 해서 앞선 질문들을 잘 대답한 혜아는 이제 마지막으로 칠판에 직접 코딩하는 문제를 받았다.
 ...
 예제 입력 2 = 20
 예제 출력 2 = long long long long long int
