...
public class No10798_AVerticalReading {
    public static void main(String[] args) throws IOException {
        // 콘솔에서 입력을 읽어오기 위한 BufferedReader 객체를 생성합니다.
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        char[][] arr = new char[5][15]; // 5행 15열의 2차원 문자 배열을 생성합니다.
        int max = 0;

        for (int i = 0; i < arr.length; i++) {  // 입력된 문자열을 읽어와 배열에 저장하는 반복문입니다.
            String str = br.readLine();
            // 현재 문자열의 길이가 최대값보다 크다면 최대값을 업데이트합니다.
            if (max < str.length()) {
                max = str.length();
            }

            for (int j = 0; j < str.length(); j++) {    // 현재 문자열의 문자들을 배열에 저장합니다.
        ...
        for (int i = 0; i < max; i ++) {   // 열을 순회하는 반복문입니다.
            for (int j = 0; j < 5; j++) {   // 열을 순회하는 반복문입니다.
                if (arr[j][i] == '\0') {    // 현재 문자가 널('\0')인지 확인합니다.
                    continue;   // 널이라면 다음 반복으로 넘어갑니다.

                }

                sb.append(arr[j][i]);   // StringBuilder에 현재 문자를 추가합니다.
...
/*
 ...
 예제 입력 1 = ABCDE
            ...
            FGHIJ
            fghij
 예제 출력 1 = Aa0FfBb1GgCc2HhDd3IiEe4Jj
 ...
 예제 입력 2 = AABCDD
            ...
            a8EWg6
            P5h3kx
 예제 출력 2 = Aa0aPAf985Bz1EhCz2W3D1gkD6x
